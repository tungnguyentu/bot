...
try:
    ...
    logging.warning(
        "TensorFlow not available. Reinforcement learning features will be limited."
    )
...
class RLTrader:
    ...
    def __init__(self, state_size=5, action_size=3, batch_size=64, episodes=100):
        ...
        if TENSORFLOW_AVAILABLE:
            ...
            logger.warning(
                "TensorFlow not available. Using random decision maker instead."
            )
    ...
    def evaluate(self, symbol, data, config):
        ...
        if not TENSORFLOW_AVAILABLE or self.model is None:
            logger.warning(
                "TensorFlow or model not available. Cannot evaluate RL model."
            )
            return None
        ...
        if len(positions) > 0:
            total_return_pct = (
                positions[-1]["equity"] / positions[0]["equity"] - 1
            ) * 100
        else:
            total_return_pct = 0
...
class TradingEnvironment:
    ...
    def step(self, action):
        ...
        prev_price = (
            self.current_price if self.current_step > 0 else self.df.iloc[0]["close"]
        )
        ...
        self.current_timestamp = (
            self.df.iloc[self.current_step]["timestamp"]
    ...
    def _get_state(self):
        ...
        if "bb_upper" in current and "bb_lower" in current and "close" in current:
            ...
            if bb_range > 0:
                bb_position = (
                    2 * (current["close"] - current["bb_lower"]) / bb_range - 1
                )
            else:
                bb_position = 0
        else:
